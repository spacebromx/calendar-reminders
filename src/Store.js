...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case actions.LOAD_REMINDER:
      const currentReminder = getReminderById(
        state.reminders,
        action.payload.id
      );
    ...
    case actions.SAVE_REMINDER:
      return {
        ...state,
        reminders: [...state.reminders, { ...action.payload, id: uniqid() }],
      };
    case actions.EDIT_REMINDER:
      const tmpIndex = state.reminders.findIndex(
    ...
    case actions.DELETE_ALL_REMINDERS:
      ...
      const filteredItems = [...state.reminders].filter(
        (item) => item.date !== newDate
      );
      return { ...state, reminders: filteredItems };
    case actions.DELETE_REMINDER:
      let preservedReminders = [...state.reminders].filter(
